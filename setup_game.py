...
def toxic_crisis() -> Engine:
    ...
    engine.game_world.commence_toxic_crisis()
    engine.do_fov = True
    ...
    engine.message_log.add_message(
        "BIG SNAKE HUNT: TOXIC CRISIS EDITION", color.welcome_text
    )
    engine.message_log.add_message(
        "Press f to shoot.\nPress x to look.\nPress i for inventory\nPress g to get\nMove with numpad."
    )
    engine.message_log.add_message(
        "You must slay all snakes. Good luck!", color.welcome_text
    )

    rifle = copy.deepcopy(entity_factories.ar_150)
    knife = copy.deepcopy(entity_factories.combat_knife)
    armor = copy.deepcopy(entity_factories.ballistic_vest)
    medkit = copy.deepcopy(entity_factories.medkit)
...
class MainMenu(input_handlers.BaseEventHandler):
    ...
    def ev_keydown(
        ...
        if event.sym in (tcod.event.KeySym.Q, tcod.event.KeySym.ESCAPE):
            ...
            return input_handlers.MainGameEventHandler(toxic_crisis())
